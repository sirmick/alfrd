...
Examples:
  ...
  %(prog)s receipt.jpg --source email --tags receipt groceries
    ...
    parser.add_argument(
        ...
        nargs='*',
        default=[],
        help='Tags for the document (e.g., company name, category). Leave empty for LLM-only tagging.'
    )
    ...
    def normalize_tag(tag: str) -> str:
        """Normalize tag to kebab-case format."""
        import re
    ...
    normalized_tags = [normalize_tag(tag) for tag in args.tags if tag]
    ...
    if normalized_tags:
        print(f"   Tags: {', '.join(normalized_tags)}")
    else:
        print(f"   Tags: (will be generated by LLM)")
